...
function consolidateApontamentos(items, dataISO) {
  ...
  for (const it of items) {
    ...
    if (ent != null) {
      if (bestEntrada == null || ent < bestEntrada || (ent === bestEntrada && (pri[it.origem] || 0) > (pri[bestEntradaOrigem] || 0))) {
        bestEntrada = ent; bestEntradaOrigem = it.origem;
      }
    }
    if (sai != null) {
      if (bestSaida == null || sai > bestSaida || (sai === bestSaida && (pri[it.origem] || 0) > (pri[bestSaidaOrigem] || 0))) {
...
function StatusBadge({ children, tone = "gray" }) {
  ...
  return <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ring-1 ring-inset ${cls}`}>{children}</span>;
}
...
function HorasTrabalhadas({ funcionarios, escalasByDia, apontByKey, apontByFuncDia, filtroFuncionario, isMobile }) {
  ...
  const horasPorFuncionario = useMemo(() => {
    const resultado = [];
    const hoje = new Date();
    const dataInicio = periodo === "hoje" ? hoje : periodo === "semana" ? startOfWeek(hoje) : new Date(hoje.getFullYear(), hoje.getMonth(), 1);
    const dataFim = periodo === "hoje" ? hoje : periodo === "semana" ? addDays(startOfWeek(hoje), 6) : new Date(hoje.getFullYear(), hoje.getMonth() + 1, 0);
    const datas = [];
    ...
    for (const func of funcionarios) {
      if (filtroFuncionario && filtroFuncionario !== "todos" && func.id.toString() !== filtroFuncionario) continue;
      let totalMinutos = 0;
  ...
  return (
    <div className="stat-card" data-accent="success" style={{ width: "100%" }}>
      ...
      <div className="stat-card__content">
        ...
        <div className="stat-title">Presentes agora</div>
        <div style={{ marginTop: 6, fontSize: 13, color: "var(--muted)" }}>
          {presentes.map(id => (funcionarios.find(f => f.id === id)?.pessoa_nome || `#${id}`)).join(", ")}
        </div>
      </div>
    </div>
  );
...
export default function DashboardAdm() {
  ...
  const apontByKey = useMemo(() => {
    ...
    for (const [k, arr] of m.entries()) {
      arr.sort((x, y) => {
        const ax = (x.entrada ?? x.saida ?? "").slice(0, 5);
        const ay = (y.entrada ?? y.saida ?? "").slice(0, 5);
  ...
  function blockStyleByMinutes(iniMin, endMin) {
    ...
    if (end != null && end <= minVisible) return { display: "none" };
            ...
            return (
              <div key={idxDia} className="dashboard-grid__day-column">
                ...
                {arrEsc.map((e, idx) => {
                  ...
                  const tone = status === "ATRASO" ? "warning" : status === "ADIANTADO" ? "info" : status === "PONTUAL" ? "success" : "success";
                  return (
                    <div key={`apo-${e.id}-${idx}`} className="dashboard-block dashboard-block--apontamento" style={{ ...style, backgroundColor: func.cor, opacity: cons.parcial ? 0.9 : 1 }} title={`Apontamento • ${func.nome} (${minutesToHHhMM(dur)}${cons.parcial ? " • em andamento" : ""})`}>
                      ...
                      <div className="dashboard-block__time">
                        ...
                        {String(cons.entradaMin % 60).padStart(2, "0")} – {cons.saidaMin != null ? `${String(Math.floor(cons.saidaMin / 60)).padStart(2, "0")}:${String(cons.saidaMin % 60).padStart(2, "0")}` : "em andamento"}
                      </div>
                ...
                {apontSoltosPorFunc.map(({ func, apList }, idx) => {
                  ...
                  const dur = (cons.saidaMin ?? cons.entradaMin) - cons.entradaMin;
                  return (
                    <div key={`apo-solto-${func.id}-${idx}`} className="dashboard-block dashboard-block--apontamento" style={{ ...style, backgroundColor: getCorFuncionario(func.id), opacity: cons.parcial ? 0.9 : 1 }} title={`Apontamento (sem escala) • ${func.pessoa_nome || func.nome}`}>
                      <div className="dashboard-block__dot" style={{ backgroundColor: "white" }} />
                      <div className="dashboard-block__name">{func.pessoa_nome || func.nome}</div>
                      <div className="dashboard-block__time">
                        {String(Math.floor(cons.entradaMin / 60)).padStart(2, "0")}:
                        {String(cons.entradaMin % 60).padStart(2, "0")} – {cons.saidaMin != null ? `${String(Math.floor(cons.saidaMin / 60)).padStart(2, "0")}:${String(cons.saidaMin % 60).padStart(2, "0")}` : "em andamento"} • fora da escala
                      </div>
                    </div>
                  );
                })}
      ...
      <div className="dashboard-mobile" role="region" aria-label={`Agenda de ${formatDateFull(diaAtual)}`}>
        ...
        <div className="dashboard-mobile__grid">
          ...
          {arrEsc.map((e, idx) => {
            ...
            const status = atrasoMin == null ? "PRESENTE" : atrasoMin > 5 ? "ATRASO" : atrasoMin < -5 ? "ADIANTADO" : "PONTUAL";
            return (
              <div key={`apo-mobile-${e.id}-${idx}`} className="dashboard-block dashboard-block--apontamento dashboard-block--mobile" style={{ ...style, backgroundColor: func.cor, opacity: cons.parcial ? 0.9 : 1 }} title={`Apontamento • ${func.nome}`}>
                <div className="dashboard-block__name">{func.nome}</div>
                <div className="dashboard-block__time">
                  {String(Math.floor(cons.entradaMin / 60)).padStart(2, "0")}:
          ...
          {apontSoltosPorFunc.map(({ func, apList }, idx) => {
            ...
            const style = blockStyleByMinutes(cons.entradaMin, cons.saidaMin);
            return (
              <div key={`apo-solto-mobile-${func.id}-${idx}`} className="dashboard-block dashboard-block--apontamento dashboard-block--mobile" style={{ ...style, backgroundColor: getCorFuncionario(func.id), opacity: cons.parcial ? 0.9 : 1 }} title={`Apontamento (sem escala) • ${func.pessoa_nome || func.nome}`}>
                <div className="dashboard-block__name">{func.pessoa_nome || func.nome}</div>
                <div className="dashboard-block__time">
                  {String(Math.floor(cons.entradaMin / 60)).padStart(2, "0")}:
                  {String(cons.entradaMin % 60).padStart(2, "0")}
                  {cons.saidaMin != null ? `–${String(Math.floor(cons.saidaMin / 60)).padStart(2, "0")}:${String(cons.saidaMin % 60).padStart(2, "0")}` : " (andamento)"} — fora da escala
                </div>
              </div>
            );
          })}
    ...
    <>
      ...
      <header className="page-header" role="region" aria-labelledby="titulo-pagina">
        <div>
          ...
          <p className="page-subtitle">{isMobile ? "Escala × Apontamento" : "Escala × Apontamento"}</p>
        </div>
