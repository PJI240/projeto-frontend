...
export default function FolhasFuncionarios() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const from = toYM(filtros.from) || filtros.from;
      const to = toYM(filtros.to) || filtros.to;
      if (from) qs.set("from", from);
      if (to) qs.set("to", to);
      if (filtros.funcionario_id !== "todos") qs.set("funcionario_id", filtros.funcionario_id);
  ...
  const filtrados = useMemo(() => {
    const f = toYM(filtros.from);
    const t = toYM(filtros.to);
  ...
  const kpis = useMemo(() => {
    let horas = 0, liquido = 0; const pessoas = new Set();
    filtrados.forEach((r) => {
  ...
  const startNovo = () => {
    setEditing(null);
    setForm({
      competencia: toYM(filtros.from) || monthISO(),
      funcionario_id: "",
      horas_normais: "",
      he50_horas: "",
      he100_horas: "",
  ...
  const salvar = async () => {
    ...
    try {
      ...
      const payload = {
        competencia: toYM(form.competencia) || form.competencia,
      ...
      if (editing) {
        await api(`/api/folhas-funcionarios/${editing.id}`, { method: "PUT", body: JSON.stringify(payload) });
        setOk("Registro atualizado.");
      } else {
        await api(`/api/folhas-funcionarios`, { method: "POST", body: JSON.stringify(payload) });
  ...
  const exportar = () => {
    ...
    const a = Object.assign(document.createElement("a"), { href: url, download: `folhas_funcionarios_${filtros.from}_a_${filtros.to}.csv` });
    ...
    <>
      ...
      <header className="page-header">
        ...
        <div className="header-actions">
          ...
          <button className="btn btn--primary" onClick={startNovo}>
      ...
      <section className="card toolbar">
        <div className="toolbar__left">
          ...
          <input type="month" className="form-control" value={filtros.from}
            onChange={(e) => setFiltros((p) => ({ ...p, from: e.target.value }))} />
          <span className="text-muted">até</span>
          <input type="month" className="form-control" value={filtros.to}
            onChange={(e) => setFiltros((p) => ({ ...p, to: e.target.value }))} />
          <select className="form-control" value={filtros.funcionario_id}
        ...
        <div className="toolbar__right">
          <input type="search" className="form-control" placeholder="Buscar por funcionário/competência..."
            value={filtros.q} onChange={(e) => setFiltros((p) => ({ ...p, q: e.target.value }))} />
          <button className="btn" onClick={() => setFiltros({ from: monthISO(), to: monthISO(), funcionario_id: "todos", q: "" })}>
      ...
      <section className="card">
        <div className="table-responsive">
          <table className="table">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={14} className="table-empty"><ArrowPathIcon className="icon-sm animate-spin" /> Carregando…</td></tr>
              ) : !filtrados.length ? (
                <tr><td colSpan={14} className="table-empty text-muted"><FunnelIcon className="icon" /> Nenhum lançamento para os filtros.</td></tr>
              ) : (
                filtrados.map((r) => (
                  <tr key={r.id}>
                    <td><code>#{r.id}</code></td>
                    <td><div className="cell-strong">{formatMonthBR(r.competencia)}</div><div className="cell-muted">{r.competencia}</div></td>
                    ...
                    <td className="text-right">
                      <div className="btn-group">
                        <button className="btn btn--icon" onClick={() => startEdit(r)} title="Editar"><PencilSquareIcon className="icon-sm" /></button>
                        <button className="btn btn--icon" onClick={() => excluir(r)} title="Excluir"><TrashIcon className="icon-sm" /></button>
                      </div>
                    </td>
                  </tr>
                ))
      ...
      {/* Modal */}
      {open && (
        <div className="modal" role="dialog" aria-modal="true" onClick={(e) => e.target === e.currentTarget && !saving && setOpen(false)}>
          <div className="modal__content">
            <header className="modal__header">
              <h2 className="modal__title">{editing ? "Editar lançamento" : "Novo lançamento"}</h2>
              <button className="btn btn--icon" onClick={() => !saving && setOpen(false)} aria-label="Fechar"><XMarkIcon className="icon-sm" /></button>
            </header>
            <div className="modal__body">
              ...
              <div className="form-grid">
                <div className="form-field">
                  ...
                  <input type="month" className="form-control" value={form.competencia}
                    onChange={(e) => setForm((p) => ({ ...p, competencia: e.target.value }))} required disabled={saving} />
                </div>
                <div className="form-field">
                  <label className="form-label">Funcionário *</label>
                  <select className="form-control" value={form.funcionario_id}
                    onChange={(e) => setForm((p) => ({ ...p, funcionario_id: e.target.value }))} required disabled={saving}>
                    <option value="">Selecione…</option>
                    {funcionarios.map((f) => (
                      <option key={f.id} value={f.id}>{f.pessoa_nome || f.nome || f.pessoa?.nome || `#${f.id}`}</option>
                    ))}
                  </select>
                </div>

                {/* Horas */}
                <div className="form-field"><label className="form-label">Horas normais</label>
                  <input className="form-control" type="number" step="0.01" value={form.horas_normais}
                    onChange={(e) => setForm((p) => ({ ...p, horas_normais: e.target.value }))} disabled={saving} />
                </div>
                <div className="form-field"><label className="form-label">HE 50% (horas)</label>
                  <input className="form-control" type="number" step="0.01" value={form.he50_horas}
                    onChange={(e) => setForm((p) => ({ ...p, he50_horas: e.target.value }))} disabled={saving} />
                </div>
                <div className="form-field"><label className="form-label">HE 100% (horas)</label>
                  <input className="form-control" type="number" step="0.01" value={form.he100_horas}
                    onChange={(e) => setForm((p) => ({ ...p, he100_horas: e.target.value }))} disabled={saving} />
                </div>

                {/* Valores */}
                <div className="form-field"><label className="form-label">Valor base</label>
                  <input className="form-control" type="number" step="0.01" value={form.valor_base}
                    onChange={(e) => setForm((p) => ({ ...p, valor_base: e.target.value, total_liquido: calcTotal({ ...p, valor_base: e.target.value }).toFixed(2) }))} disabled={saving} />
                </div>
                <div className="form-field"><label className="form-label">Valor HE 50%</label>
                  <input className="form-control" type="number" step="0.01" value={form.valor_he50}
                    onChange={(e) => setForm((p) => ({ ...p, valor_he50: e.target.value, total_liquido: calcTotal({ ...p, valor_he50: e.target.value }).toFixed(2) }))} disabled={saving} />
                </div>
                <div className="form-field"><label className="form-label">Valor HE 100%</label>
                  <input className="form-control" type="number" step="0.01" value={form.valor_he100}
                    onChange={(e) => setForm((p) => ({ ...p, valor_he100: e.target.value, total_liquido: calcTotal({ ...p, valor_he100: e.target.value }).toFixed(2) }))} disabled={saving} />
                </div>
                <div className="form-field"><label className="form-label">Proventos (+)</label>
                  <input className="form-control" type="number" step="0.01" value={form.proventos}
                    onChange={(e) => setForm((p) => ({ ...p, proventos: e.target.value, total_liquido: calcTotal({ ...p, proventos: e.target.value }).toFixed(2) }))} disabled={saving} />
                </div>
                <div className="form-field"><label className="form-label">Descontos (-)</label>
                  <input className="form-control" type="number" step="0.01" value={form.descontos}
                    onChange={(e) => setForm((p) => ({ ...p, descontos: e.target.value, total_liquido: calcTotal({ ...p, descontos: e.target.value }).toFixed(2) }))} disabled={saving} />
                </div>

                <div className="form-field"><label className="form-label">Total líquido</label>
                  <input className="form-control" type="number" step="0.01" value={form.total_liquido}
                    onChange={(e) => setForm((p) => ({ ...p, total_liquido: e.target.value }))} disabled={saving} />
                </div>

                <div className="form-field"><label className="form-label">Inconsistências</label>
                  <input className="form-control" type="number" min="0" value={form.inconsistencias}
                    onChange={(e) => setForm((p) => ({ ...p, inconsistencias: Number(e.target.value) }))} disabled={saving} />
                </div>
              </div>

              <p className="muted" style={{ marginTop: 8 }}>
                O total líquido é recalculado automaticamente ao alterar os campos de valores.
              </p>
            </div>
            <footer className="modal__footer">
              <button className="btn" onClick={() => setOpen(false)} disabled={saving}>Cancelar</button>
              <button className="btn btn--primary" onClick={salvar} disabled={saving || !form.funcionario_id || !form.competencia} aria-busy={saving}>
