...
export default function Apontamentos() {
  ...
  useEffect(() => {
    loadItens();
  }, [periodo, funcionarioId, origem]); // eslint-disable-line
  ...
  function parseCSV(text) {
    ...
    for (const line of lines) {
      const [funcionario_id, data, turno_ordem, entrada, saida, origem, obs] = line
        .split(";")
        .map((s) => s?.trim());
  ...
  function onBuildPreview() {
    ...
    const conflitos = p.validas.filter((v) =>
      existentes.has(`${v.funcionario_id}|${v.data}|${v.turno_ordem}|${v.origem}`)
    );

    const validasSemConflito = p.validas.filter(
      (v) => !existentes.has(`${v.funcionario_id}|${v.data}|${v.turno_ordem}|${v.origem}`)
    );

    setPreview({
      validas: validasSemConflito,
  ...
  const importarValidas = async () => {
    ...
    try {
      ...
      setSuccess(
        `Importadas ${preview.validas.length} linhas. Recusadas ${
          preview.invalidas.length + preview.conflitos.length
        }.`
      );
      setShowImport(false);
      setCsvText("");
  ...
  // Agrupamento para relatório (por funcionário -> por data)
  const relatorio = useMemo(() => {
    const byFunc = new Map();
    for (const it of itens) {
      const tot = duracao(it.entrada, it.saida);
      const funcId = it.funcionario_id;
      if (!byFunc.has(funcId)) byFunc.set(funcId, { linhas: [], totalMin: 0 });
      byFunc.get(funcId).linhas.push({
        data: it.data,
        turno: it.turno_ordem,
        entrada: it.entrada || "-",
        saida: it.saida || "-",
        minutos: tot,
        origem: it.origem || "",
      ...
      byFunc.get(funcId).totalMin += tot;
    }

    // ordenar linhas por data/turno
  ...
  function exportCSV() {
    ...
    for (const [funcId, info] of relatorio.entries()) {
      ...
      rows.push(
        [
          funcId,
          `"${getFuncionarioRotulo(funcId).replace(/"/g, '""')} (Subtotal)"`,
          "",
          "",
          "",
          "",
          info.totalMin,
          fmtHHMM(info.totalMin),
          "",
          "",
        ].join(";")
      );
    ...
    const nome = funcionarioId
      ? `apontamentos_${funcionarioId}_${periodo.de}_a_${periodo.ate}.csv`
      : `apontamentos_${periodo.de}_a_${periodo.ate}.csv`;
    downloadBlob(csv, nome, "text/csv;charset=utf-8");
  }

  function exportPDF() {
    // Gera HTML simples e acessível para impressão
    ...
    const style = `
      <style>
        :root{
          --fg:#111; --muted:#555; --border:#e5e7eb; --focus:#ffbf47;
        }
        body { font-family: system-ui, -apple-system, Segoe UI, Roboto, Ubuntu, Cantarell, Noto Sans, Helvetica, Arial, "Apple Color Emoji", "Segoe UI Emoji"; color: var(--fg); padding: 24px; }
    ...
    let body = `
      <h1>${titulo}</h1>
      <p class="sub">Período: ${periodo.de} a ${periodo.ate}${
            ...
            ${info.linhas
              .map(
                (l) => `<tr>
                  ...
                  <td>${l.entrada}</td>
                  <td>${l.saida}</td>
                  <td>${fmtHHMM(l.minutos)}</td>
                  <td>${l.origem}</td>
                  <td>${l.obs ? String(l.obs).replace(/</g, "&lt;").replace(/>/g, "&gt;") : ""}</td>
                </tr>`
              )
              .join("")}
          </tbody>
    ...
    win.document.close();
    // Abre diálogo de impressão (usuário pode salvar em PDF)
    win.focus();
    win.print();
  }
  ...
  return (
    <>
      ...
      <header className="page-header" role="region" aria-labelledby="titulo-pagina">
        ...
        <div className="page-header__toolbar" aria-label="Ações da página">
          <button
            ...
            onClick={novoAjuste}
            aria-label="Criar ajuste de apontamento"
          >
          ...
          <button
            className="btn btn--neutral"
            onClick={exportCSV}
            aria-label="Exportar relatório (CSV/planilha)"
          >
            <DocumentArrowDownIcon className="icon" aria-hidden="true" />
                ...
                <tbody>
                  {itens.map((it) => {
                    ...
                    const inconsistencia =
                      it.entrada && it.saida && minutes(it.saida) < minutes(it.entrada);
                    return (
          ...
          <div className="form-container" style={{ maxWidth: "800px", maxHeight: "90vh" }}>
            ...
            <div className="form">
              <p style={{ marginTop: 0, color: "var(--muted)" }}>
                Formato: <code>funcionario_id;data;turno_ordem;entrada;saida;origem;obs</code> — datas{" "}
                <code>YYYY-MM-DD</code>, horas <code>HH:MM</code>.
              </p>
              <div className="form-field span-2">
          ...
          <div className="form-container" style={{ maxWidth: 680 }}>
            ...
            <div className="form">
              <p className="page-subtitle" style={{ marginTop: 0 }}>
                Esta ação <strong>não edita</strong> o apontamento original. Ela o{" "}
                <strong>invalida por tratamento</strong> e cria um <strong>AJUSTE</strong> para a pessoa correta.
              </p>
              <div className="form-grid">
                <div className="form-field">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setTratamento((t) => ({ ...t, destino_funcionario_id: e.target.value }))
                    }
                    required
                  >
                    <option value="">Selecione…</option>
                ...
                <div className="form-field span-2" style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <input
                    ...
                    onChange={(e) =>
                      setTratamento((t) => ({ ...t, invalidar_original: e.target.checked }))
                    }
                  />
      ...
      {/* CSS local apenas para pequenos ajustes de layout responsivo */}
      <style jsx>{`
        ...
        .row-inconsistente { background: rgba(220, 38, 38, 0.05) !important; }
        ...
        .badge--warning { background: rgba(234, 179, 8, 0.2); color: #92400e; border-color: rgba(234, 179, 8, 0.35); }
        ...
        .form-field input, .form-field select, .form-field textarea {
          min-height: 44px; padding: 10px 12px; border: 1px solid var(--border); border-radius: 12px; background: #fff; color: #111; font-size: var(--fs-16);
        }
